...
int boltzmann(Cosmology *cosmo, double *zlist_transfer); //this calls the Boltzmann solver

//// Read the input file, and store the values:
int read_input_file (char *filename, double parameter_values[Ninput]);

double read_param (std::string buffer, std::string parameter_name); //returns parameter value from buffer
