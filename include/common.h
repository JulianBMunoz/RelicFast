	...
	#define boltzmann_tag  _CLASS_ //0 for CLASS, 1 for CAMB. CLASS needed in most cases.
	#define run_boltzmann_option   _TRUE_  //0 if no, otherwise YES. If choosing more than on z_collapse have to set to 1.
...
//the inputs are read with read_input_file(), and Cosmology is initialized with prepare_cosmology(),
	struct Cosmology{
...
//parameters read as input
		...
		double Neff_input; //N_effective read, we will subtract 1 per each massive neutrino to get Neff.


//derived paramters:

		double Neff; // from Neff_input subtracting neutrinos that are massive and so on.
		...
		double OmegaM; //CMB+b energy density (NOT INCLUDING RELICS/NUs!). Used for halo masses
		...
		double OmegaR;// the part that is radiation (at all z)
		 ...
		 double OmegaL; //Omega_Lambda, we close the Friedmann Eq. assuming flat Universe.
...
//read:
		...
		double ktop; //Mpc-1, maximum and minimum k over which we calculate things.
		double kbot;
