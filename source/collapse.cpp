...
#define option_initial_velocity 0 
//0 for sigma_dot/sigma, 1 for T_dot/T (@k_star=2/Mpc). 
//They agree, but it's a good sanity check.

int collapse(Cosmology *cosmo, double *zlist_transfer){
    //one of the main functions, computes \delta_crit at collapse for Mhalo at
    //zcollapse. zlist_transfer is the redshift list we ran Boltzmann code for
    //(since CLASS does not save zs, although CAMB/AXIONCAMB does)
    ...
    //we stablish if for the light relic and Mhalo we have to do clustering or
    //not. Threshold given by mnu_min_clustering @ Mhalo=Mhalo_min_clustering.
    int i_clustering_index;
    int do_clustering;

    ///////////////////////////////////////////////////////////////////////////
    ////we READ the boltzmann output to get the transfer functions between z=zi
    ////and z=0.    
    ///////////////////////////////////////////////////////////////////////////

    double *k_transfer_array; 
    // k for transfer functions (common for all) (NOT over h)

    double **transfer_matter;
    double **transfer_nu_massless;
    double **transfer_nu1;
    double **transfer_gamma;
    // transfer functions (not over k^2), for matter, massless nu, and photons.
    ...
    //have to create these arrays anyway, to read transfer files. They can be 
    //nonsense if mnu2=0 or no extra species, is ok.
    ...
    if(boltzmann_tag == _CLASS_){ //CLASS
        transfer_check=save_transfers_class(
            cosmo, 
            zlist_transfer, 
            k_transfer_array, 
            transfer_matter, 
        ...
        transfer_check=save_transfers_camb(
            cosmo, 
            zlist_transfer, 
            k_transfer_array, 
            transfer_matter, 
    ...
    const double kmax=k_transfer_array[length_transfer-1];    
    //printf("kmin=%.1le, kmax=%.1le \n",kmin, kmax);

    //find the closest value to k_input in k_transfer_array to avoid unnecesary
    // interpolation.
    long jk;
    ...
    for(i=0;i<cosmo->N_klong;i++){
        jk=find_value(
            length_transfer, 
            k_transfer_array, 
            cosmo->klong_list_input[i]
        );
        klong_list[i]=k_transfer_array[jk];
        //printf("j=%ld, out=%le, in=%le \n", jk, klong_list[i],cosmo->klong_list_input[i]);
    }

    //We compute T_dot to have two different initial conditions for comparison.
    double *d_transfer_array_zi; 
    //dT/dz. The matter(c+b) transfer function and k/h obtained from CAMB code
    ...
    const double zip1=zlist_transfer[index_zip1]; 
    ...
    for(j=0;j<length_transfer;j++){
        d_transfer_array_zi[j] = (
            (transfer_matter[index_zip1][j]-transfer_matter[index_zip2][j])
            / (zip1-zip2)
        ); 
        //dT/dz@zi
    }
    ...
    //T*k^2, k: The matter(c+b) transfer function and k/h obtained from CAMB 
    // code, and k (NOT over h)
    ...
    double *transfer_array_zip1;
    double *transfer_array_zip2; 
    //for CDM+b, derivative

    double *transfer_array_gamma_zi;
    double *transfer_array_nu_massless_zi;
    double *transfer_array_nu1_zi; 
    ...
    transfer_check=gettransfer_gamma(
        cosmo, 
        filename,  
        k_transfer_array, 
        transfer_array_gamma_zi
    );
    ...
    const int number_collapse=find_value(
        cosmo->N_zcoll,
    ...
    if (debug_mode>0){
        printf(
            "number_collapse=%d, j_collapse=%d \n",
            number_collapse,
            j_collapse
        );
    }
    ...
    const double zmin_EoS=0.;
    //we go down to z=0, needed to take ratios with Omega.
    ...
    for(i=0;i<Nz_EoS;i++){
        ...
        //z linearly from 0 to zi EQUALLY SPACED! 
        ...
        plist_nu1_EoS[i] = pressure_WDM(cosmo->mnu1,Temp_nu);        
        ...
        if(debug_mode > 0){
            fprintf(    
                ...
                "%le %le %le \n",
                zlist_EoS[i], 
                rholist_nu1_EoS[i]/rholist_nu1_EoS[0], 
                plist_nu1_EoS[i]/rholist_nu1_EoS[i]
            );
        }
    }
    ...
    if(cosmo->Omeganu2>0){
        for(i=0;i<Nz_EoS;i++){
            ...
            Temp_nu = cosmo->T0_nu * (1.+zlist_EoS[i]); 
            //neutrino temperature in K

            rholist_nu2_EoS[i] = density_WDM(cosmo->mnu2,Temp_nu);
            plist_nu2_EoS[i] = pressure_WDM(cosmo->mnu2,Temp_nu);
            //printf("z=%.1le,  w=%.1le, Tnu/mnu=%.1le \n",zlist_EoS[i],plist_EoS[i]/rholist_EoS[i],Temp_nu*KtoeV/(mnu1));
            ...
            if(debug_mode > 0){
                fprintf(
                    ...
                    "%le %le %le \n",
                    zlist_EoS[i], 
                    rholist_nu2_EoS[i]/rholist_nu2_EoS[0], 
                    plist_nu2_EoS[i]/rholist_nu2_EoS[i]
                );
            }
        }
    ...
    if(cosmo->Omega_extra>0){
        for(i=0;i<Nz_EoS;i++){
            ...
            Temp_extra = cosmo->T0_extra * (1.+zlist_EoS[i]); 
            //neutrino temperature in K

            rholist_extra_EoS[i] = density_WDM(cosmo->m_extra,Temp_extra);
            plist_extra_EoS[i] = pressure_WDM(cosmo->m_extra,Temp_extra);
            //printf("z=%.1le,  w=%.1le, Tnu/mnu=%.1le \n",zlist_EoS[i],plist_EoS[i]/rholist_EoS[i],Temp_nu*KtoeV/(mnu1));
            ...
            if(debug_mode > 0){
                fprintf(
                    ...
                    "%le %le %le \n",
                    zlist_EoS[i], 
                    rholist_extra_EoS[i]/rholist_extra_EoS[0], 
                    plist_extra_EoS[i]/rholist_extra_EoS[i]
                );
            }
        }
    ...
    if(boltzmann_tag == _AXIONCAMB_){
        for(i=0;i<Nz_EoS;i++){
            ...
            //printf("%le\n", cosmo->axion_N);
            //printf("%.10e\n", zlist_EoS[0]);
            rholist_axion_EoS[i] = interpol(cosmo->axion_rho, cosmo->axion_z, 2*(*cosmo->axion_N), zlist_EoS[i]);
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    const double logz_solution_max=log(cosmo->z_collapse/2);
    //as in collapse code.

    const double dlogz_solution=(
        (logz_solution_max-logz_solution_min)
        / (Nz_solution-1)
    );
    ...
    if(debug_mode>0) printf(
        "coll_index= %d, z_coll = %lf \n", 
        coll_index, 
        zlist_transfer[coll_index]
    );
    const double sigma8_collapse = getsigma_8(
        cosmo, 
        k_transfer_array, 
        transfer_matter[coll_index]
    );
    const double sigma8_i = getsigma_8(
        cosmo, 
        k_transfer_array, 
        transfer_matter[index_zi]
    );

    //we set delta_short.
    const double delta_short_min_constant=(
        delta_short_constant_centroid
        / (2.0+boost_initial_conditions)
    ); 
    //initial conditions, to do bipartition. these are the benchmark.
    const double delta_short_max_constant=(
        delta_short_constant_centroid
        * (2.0+boost_initial_conditions)
    );
    ...
    const double tolerance_ini= (
        tolerance 
        + (do_clustering>0)*tolerance*4.0
    ); 
    ...
    for(int i_klong=0;i_klong<cosmo->N_klong;i_klong++){ 
        //we find transfer function at initial redshift
        double k_long = klong_list[i_klong];
        Ti_klong[i_klong]=interpol_2D(
            transfer_matter, 
            zlist_transfer, 
            Nz_transfer, 
            k_transfer_array, 
        ...
        for(i=0;i<Nz_transfer;i++){
            z=zlist_transfer[i];
            transfer_gamma_klong[i_klong][i]=interpol_2D(
                transfer_gamma, 
                zlist_transfer, 
                Nz_transfer, 
                k_transfer_array, 
            ...
            transfer_nu_massless_klong[i_klong][i]=interpol_2D(
                transfer_nu_massless, 
                zlist_transfer, 
                Nz_transfer, 
                k_transfer_array, 
            ...
            transfer_nu1_klong[i_klong][i]=interpol_2D(
                transfer_nu1, 
                zlist_transfer, 
                Nz_transfer, 
                k_transfer_array, 
    ...
    for(int i_klong=0;i_klong<cosmo->N_klong;i_klong++){
        ...
        for(i=0;i<Nz_transfer;i++){
            z=zlist_transfer[i];
            transfer_nu2_klong[i_klong][i]=interpol_2D(
                transfer_nu2, 
                zlist_transfer, 
                Nz_transfer, 
                k_transfer_array, 
    ...
    int iM;
    const int N_klong_calc [N_delta_long] = {1, cosmo->N_klong}; 
    //what k_longs we calculate over. For delta_long=0 only one is needed, 
    //otherwise one per kmode.
    //N_delta_long is defined in common.h

    double *delta_long_list;
    delta_long_list=allocate_1D_array(N_delta_long);

    const double delta_long_min=0.; 
    //the maximum value is specified in common.h for convenience to change it.
    ...
    //delta_short_critical, extrapolated to z_collapse.
    double **delta_long_collapse; 
    //delta_long, extrapolated to z_collapse.
    ...
    double T_z_collapse_klong; 
    //transfer function T(k_long) at zcollapse, to extrapolate \delta_long.
    ...
    for(iM=0;iM<cosmo->N_Mhalo;iM++){
        ...
        //sigma(M) at zi, z)collapse, and derivative (and z=0 just in case), 
        //for initial conditions of ODE. (option_initial_velocity=0)
        ...
        ); //Mpc-1; where we evaluate Tdot/T.

        //we find the k that is closest to kstar, since that is easier than 
        //interpolating for no reason.
        ...
        double T_z_collapse_kstar=transfer_array_z_collapse[indexkstar]; 
        //transfer function T(k_pivot) at zcollapse, to extrapolate \delta_crit.
        ...
        #ifdef _OPENMP
            omp_set_nested(1); 
        //we need to nest the two parallel loops since the limits on the inner
        //for() depend on the outer one, so the scheduler does not know how to 
        //do collapse(2).
        ...
        for(int i_delta_long=0;i_delta_long<N_delta_long;i_delta_long++){
            ...
            for(int i_klong=0;i_klong<N_klong_calc[i_delta_long];i_klong++){
                ...
                //long-wavelength perturbation for CDM+b , for photons , and 
                //for massless neutrinos.

                double k_long=klong_list[i_klong];

                double ddelta_long_dt=(
                    delta_long*(-Hi*(1.+zi))
                    *dTi_klong[i_klong]/Ti_klong[i_klong]
                ); //d delta_long/dt, calculated as dT/dt*1/T*delta_long.

                double delta_short_min=delta_short_min_constant; 
                //we restart the initial conditions, and redo the procedure.

                double delta_short_max=delta_short_max_constant;
                double delta_short=0.0,ddelta_short_dt; 
                //private copies for each run
                double Ri, Ridot, Rpi, z_coll_iteration=0.0; 
                //initial conditions and z of collapse of each iteration.

                if(debug_mode>1){
                    printf(
                        "deltashort_min=%.2le, deltashort_max=%.2le \n", 
                        delta_short_min, 
                        delta_short_max
                    );
                }
                ...
                for(;delta_short_max/delta_short_min-1.>tolerance_ini;){
                    delta_short=(delta_short_max+delta_short_min)/2.0; 
                    //bisection, we update the min or max value.

                    if(option_initial_velocity==0){
                        ///Option 1: through sigma(M)
                        ddelta_short_dt=(
                            delta_short*(-Hi*(1.+zi))*d_sigmaM_i/sigmaM_i
                        ); 
                        //since: ddelta_dt/delta = dsigma_dt/sigma = -H*(1+z) 
                        //dsigma_dz/sigma
                    }
                    else if(option_initial_velocity==1) {
                        ///Option 2: through transfer function (they agree)
                        ddelta_short_dt=(
                            delta_short*(-Hi*(1.+zi))*dTi_kstar/Ti_kstar
                        ); 
                        //since:ddelta_dt/delta = dT_dt/T = -H*(1+z) dT_dz/sigma
                    }
                    else {
                        printf("ERROR: option_initial_velocity has to be \
                            either 0 or 1 \n");
                        exit(0);
                    }

                    //these are the i.c. we will feed the function
                    //find_z_collapse_XXX.

                    Ri= Ribar*(1.0-1./3.*(delta_short+delta_long)); 
                    //R_i in Mpc.

                    Ridot=Ri*Hi*(1.0-1./3.*(ddelta_short_dt+ddelta_long_dt)/Hi); 
                    //dR/dt in Mpc/Mpc
                    ...
                    if(debug_mode > 0){
                        printf(
                            "Ri=%.3le, dRi/dz=%.3le, delta_short=%.2le \n", 
                            Ri, 
                            Rpi, 
                            delta_short
                        );
...
//                    }
                    ...
                    if(cosmo->Omega_extra+cosmo->Omeganu2+cosmo->Omeganu1==0){
                        //nothing extra
                        if (debug_mode>0) printf(
                            "Collapse with massless neutrinos and axion \n"
                        );

                        z_coll_iteration=find_z_collapse_masslessnu_axion(
                            cosmo, 
                            Ri, 
                            Rpi, 
                            delta_long, 
                ...
                if(debug_mode > 0){
                    printf(
                        "delta_short=%le, delta_long=%le (with k_long=%le) \n",
                        delta_short, 
                        delta_long, 
                        k_long
                    );
                ...
                if(abs(z_coll_iteration-cosmo->z_collapse)>tolerance_z){ 
                    //collapses too quickly
                    printf("Not converged to z_collapse. Initial \
                        conditions too narrow, make \
                        boost_initial_conditions bigger. \n"
                    );
                    printf(
                        "z_coll_it=%.3le and z_collapse=%.3le \n",
                        z_coll_iteration,
                        cosmo->z_collapse
                    );
                }
        ...
        for(int i_delta_long=0;i_delta_long<1;i_delta_long++){
            for(int i_klong=1;i_klong<cosmo->N_klong;i_klong++){
                delta_short_collapse[i_delta_long][i_klong]=(
                    delta_short_collapse[i_delta_long][0]
                );
            }
        }
        ...
        if (debug_mode > 1){
            for (i=0;i<Nz_solution;i++){
                printf(
                    "i=%d, z=%.1le, R(z)/Mpc=%.1le \n",
                    i, 
                    logz_solution_min*exp(dlogz_solution*i),
                    Rhalo_solution[i]
                );
            }
        }
        ...
        if(do_clustering == 1 && (
            cosmo->Omega_extra + cosmo->Omeganu2 + cosmo->Omeganu1 > 0)){

            for(
                i_clustering_index=0;
                i_clustering_index<Nrecusions_nu_clustering;
                i_clustering_index++
                ){
                ...
                if(
                    ...
                    (
                        cosmo->mnu1 > 
                        mnu_min_clustering*sqrt(
                            Mhalo_min_clustering/cosmo->Mhalo
                        )
                    ) 
                    ){

                    nuclustering_check=findmass_1nu(
                        cosmo, 
                        cosmo->mnu1, 
                        cosmo->T0_nu, 
                        zmin_EoS, 
                ...
                if(
                    ...
                    (
                        cosmo->mnu2 > 
                        mnu_min_clustering
                        * sqrt(Mhalo_min_clustering/cosmo->Mhalo)
                    )){
                ...
                // and we do the same thing now that we have calculated the 
                //nu_collapse, but with more precision.

                #pragma omp parallel for
                for(
                    int i_delta_long=0;
                    i_delta_long<N_delta_long;
                    i_delta_long++){
                    ...
                    for(
                        ...
                        double delta_short_min=delta_short_min_constant; 
                        //we restart the initial conditions, and redo the 
                        //procedure.

                        double delta_short_max=delta_short_max_constant;
                        double delta_short,ddelta_short_dt; 
                        //private copies for each run
                        double Ri, Ridot, Rpi, z_coll_iteration; 
                        //initial conditions and z of collapse of each 
                        //iteration.


                        for(;delta_short_max/delta_short_min-1.>tolerance;){
                            delta_short=(delta_short_max+delta_short_min)/2.; 
                            //bisection, we update the min or max value.
                            ...
                            if(option_initial_velocity==0){
                                ...
                                //since:ddelta_dt/delta = 
                                //dsigma_dt/sigma = -H*(1+z) dsigma_dz/sigma
                            }
                            else if(option_initial_velocity==1) {
                                ///Option 2: through transfer 
                                //function (they agree)
                                ddelta_short_dt=(
                                    delta_short
                                    * (-Hi*(1.+zi))
                                ...
                                //since:ddelta_dt/delta = 
                                //dT_dt/T = -H*(1+z) dT_dz/sigma
                            }
                            else {
                                printf("option_initial_velocity has to be \
                                    either 0 or 1 \n");
                                exit(0);
                            }

                            //these are the i.c. we will feed the function 
                            //find_z_collapse_long.
                            ...
                            Ridot= (
                                ...
                                * (
                                    1.0
                                    - 1./3.
                                    * (ddelta_short_dt+ddelta_long_dt)/Hi)
                            ); 
                            //dR/dt in Mpc/Mpc
...
//                            }
                            if(cosmo->Omega_extra + cosmo->Omeganu2 + cosmo->Omeganu1 == 0.){
                                if (debug_mode>0) printf(
                                    "Collapse with massless nu & axion \n"
                                );
                                z_coll_iteration=find_z_collapse_masslessnu_axion(
                                    cosmo,
                                    Ri,
                                ...
                                if (debug_mode>0) printf(
                                    "Collapse with nu1,nu2 & extra & axion \n"
                                );
                                z_coll_iteration=find_z_collapse_3nu_axion(  
                                    cosmo, 
                                    Ri, 
                        ...
                        if(abs(z_coll_iteration-cosmo->z_collapse)>tolerance_z){ 
                            //collapses too quickly
                            printf("Not converged to z_collapse (after doing \
                                Mnu_collapse). Initial conditions too narrow,\
                                make delta_short_max and delta_short_min \
                                wider. \n"
                            );
                            printf(
                                "z_coll_it=%.3le and z_collapse=%.3le \n",
        ...
        // since the delta_long=0 does not depend on k we can just copy it for 
        //all ks (again).
        for(int i_delta_long=0;i_delta_long<1;i_delta_long++){
            for(int i_klong=1;i_klong<cosmo->N_klong;i_klong++){
                delta_short_collapse[i_delta_long][i_klong]=(
        ...
        for(int i_delta_long=0;i_delta_long<N_delta_long;i_delta_long++){
            for(int i_klong=0;i_klong<cosmo->N_klong;i_klong++){
                ...
                T_z_collapse_klong=interpol_2D(
                    transfer_matter, 
                    zlist_transfer, 
                    Nz_transfer, 
                    k_transfer_array, 
                ...
                delta_long_collapse[i_delta_long][i_klong]=(
                    delta_long*T_z_collapse_klong/Ti_klong[i_klong]
                );    
                //this is right as well 
                //printf("klong=%.1le, delta_long_collapse/delta_long= %.3le
        ...
        if(print_headers!=0){
            fprintf(
                fp,
                "delta_long_initial   k_long[1/Mpc]   delta_short_initial \n"
            );
        }
...
double find_z_collapse_nothing(
    ...
    // we read the instantaneous transfer function from CLASS/CAMB at each 
    // redshift.
    ...
    double OmG, Omnu_massless; 
    //photon, massless and massive neutrino energy density.
    ...
    OmG= OmGbar * (1.+delta_long*T_gamma/T_matter); 
    //Omega_gamma(z) (1+\delta_long_gamma)
    Omnu_massless= Omnu_masslessbar * (1.+delta_long*T_nu/T_matter); 
    //Omega_massless neutrino(z) (1+\delta_long_nu)
    double OmR2 = OmG + Omnu_massless;

    double z_next, dE2; 
    //for the next values, to do Heun's method. We assume Omega_L does 
    //not change.
    ...
    for(i=1; i<npoints-1 && R2>0.; i++){
        ...
        R1 = R2;
        Rp1 = Rp2;
        //including all z-dependent quantities, for Heun's method. Remember 2 
        //means next step.
        z=z_next;
        H=H2;
        OmR=OmR2;
        ...
        - R1/2.*(2*OmR - 2*OmL)*cosmo->H0_Mpc*cosmo->H0_Mpc/pow(H*(1.+z),2.); 
        //R''(R1,t1)
        ...
        //these are too annoying to keep even with debug_mode, 
        //activate manually if you want to debug:
        //printf("z=%le, H=%le , H_LCDM=%le , dE=%le , dE_LCDM=%le\n", z, 
        //H, cosmo->H0_Mpc*E_LCDM(cosmo, z), dE, dlogE_dz_LCDM(cosmo, z));
...
double find_z_collapse_1nu(
    ...
    double * const transfer_gamma_klong, 
    double * const transfer_nu_klong, 
    double * const transfer_nu_massive_klong,
    const double zmin_EoS,      
    const double dz_EoS, 
    const long Nz_EoS, 
    double * const rholist_EoS, 
    double * const plist_EoS,
    ...
    // This function returns the z at which an overdensity collapses. global 
    //cosmological parameters assumed.
    // In the presence of a long-wavelength PHOTON, MASSLESS and 1 MASSIVE 
    //NU (OR OTHER EXTRA) perturbation.
    //we read the instantaneous transfer function from CAMB at each redshift.
    //and we save R(z) to perform BKT calculation of neutrino clustering, or we
    //input Mnu(z) if we already calculated it.
    ...
    //the nu_massive can be any new species, just make sure that the Omegas 
    //are well defined here
    if((cosmo->Omega_extra>0) && (cosmo->mnu1>0)){
        printf(
            "Using wrong collapse function, XXX_1nu \
            only accepts one species. \n"
        );
    }
    ...
    double rhonu1_z=interpol_cubic(zmin_EoS, dz_EoS, rholist_EoS, Nz_EoS, zi); 
    //rho of nu1 at z
    double rho_ratio_nu1=rhonu1_z/rhonu1_0; //ratio to rho of zero
    double Omnu1bar_z =  (
        cosmo->Omeganu1 * rho_ratio_nu1 + cosmo->Omega_extra * rho_ratio_nu1
    ); //omeganu1
    ...
    double OmG= OmGbar * (1.0+delta_long*T_gamma/T_matter); 
    //Omega_gamma(z) (1+\delta_long_gamma)
    double Omnu_massless= Omnu_masslessbar * (1.0+delta_long*T_nu/T_matter); 
    //Omega_massless neutrino(z) (1+\delta_long_nu)
    double OmR, OmR2 = OmG + Omnu_massless;
    //and initial perturbation in nu1    (massive) and pressure
    double pnu1_z = interpol_cubic(zmin_EoS, dz_EoS, plist_EoS, Nz_EoS, zi); 
    //pressure of nu1 at z
    double wnu1_z = pnu1_z/rhonu1_z; //equation of state
    double T_nu1_z=interpol(
        transfer_nu_massive_klong, 
        zlist_transfer, 
        Nz_transfer, 
        zi
    );
    ...
    double H2 = cosmo->H0_Mpc* sqrt(OmL + OmM + OmRbar + Omnu1bar_z); 
    // H(z_next)
    ...
    for(i=1; i<npoints-1 && R2>0.; i++){
        ...
        if (i % collapse_steps == 0){
            ...
            if(debug_mode > 1){
                printf(
                    "z=%.1le, R=%.1le, Mnu/Mhalo=%1le \n\n", 
                    z, 
                    R_solution[i_solution], 
                    Mhalo_nu1_Mpc/cosmo->Mhalo_Mpc
                );
            }
        }
        ...
        Rpp1 = (- Rp1*(1./(1+z) + dE)
            ...
            - R1/2.*(
                2*OmR 
                + (1.+3.*wnu1_z + delta_nu1_z*(1+3.0*csq_ad_nu1_z))*Omnu1bar_z 
                - 2*OmL
        ...
        csq_ad_nu1_z = wnu1_z2 + d_wnu1_z/3.0/(1.0+wnu1_z2)*(1.0+z_next); 
        //adiabatic sound speed squared.

        H2  = cosmo->H0_Mpc * sqrt(OmL + OmM + OmRbar + Omnu1bar_z);// H(z_next)

        dE2 = (H2-H)/H2/zstep_lin; 
        //dE/dz*1/E (z_next). We update the stored value, although 
        //it's unnecessary.

        if(debug_mode > 1){ 
            //check that derivatives are well defined, it should be for any 
            //reasonable value of npoints, but just in case:
            printf(
                "w(z1)-w(z2)=%.3le , w(z2)=%.3le , dw/dz=%.3le \n", 
                wnu1_z-wnu1_z2, 
                wnu1_z2, 
                d_wnu1_z
            );
            printf(
                "H(z1)-H(z2)=%.3le , H(z2)=%.3le , dH/dz=%.3le \n", 
                H-H2, 
                H2, 
                dE2*H2
            );
            printf(
                "z=%.3le, cs^2/w(z2)=%.3le \n", 
                z_next, 
                csq_ad_nu1_z/wnu1_z2
            ); //cs^2 and w are within 10% of each other.
        }


        //these are too annoying to keep even with debug_mode, activate 
...
double find_z_collapse_2nu(
    ...
    // This function returns the z at which an overdensity collapses. global 
    //cosmological parameters assumed.
    // In the presence of a long-wavelength PHOTON, MASSLESS and 2 MASSIVE 
    //NU (or 1 nu + 1 extra) perturbation.
    //we read the instantaneous transfer function from CAMB at each redshift.
    ...
    if((cosmo->Omega_extra>0) && (cosmo->mnu2>0)){
        printf(
            "Using wrong collapse function, XXX_2nu only accepts nu1 + one \
            other species. \n"
        );
    }
    ...
    double Omnu2bar_z =  (
        cosmo->Omeganu2 * rho_ratio_nu2 + cosmo->Omega_extra * rho_ratio_nu2
    ); //omeganu2 or omega extra, only one!

    double H = (
        cosmo->H0_Mpc*sqrt(
        OmL + OmM + OmRbar + Omnu1bar_z + Omnu2bar_z
        )
    ); //H(zi)
    ...
    for(i=1; i<npoints-1 && R2>0.; i++){
        ...
        //we save R and read Mnu, only every few steps since it 
        //doesn't vary much.
        if (i % collapse_steps == 0){
            R_solution[i_solution]=R1;
            Mhalo_nu1_Mpc = Mnu1_solution[i_solution];
...
double find_z_collapse_3nu(
    ...
    double H = cosmo->H0_Mpc* sqrt(
        OmL + OmM + OmRbar + Omnu1bar_z + Omnu2bar_z + Omnu3bar_z
    ); //H(zi)
...
double find_z_collapse_masslessnu_axion(
    ...
    // This function returns the z at which an overdensity collapses. global 
    //cosmological parameters assumed.
    // In the presence of a long-wavelength PHOTON and MASSLESS 
    //we read the instantaneous transfer function from CAMB at each redshift.
    //and we save R(z) to perform BKT calculation of neutrino clustering, or 
    //we input Mnu(z) if we already calculated it.
    ...
    double H = cosmo->H0_Mpc* sqrt(OmL + OmM + OmRbar + Omaxionbar_z); //H(zi)
    ...
    //add initial perturbation in axion    (massive) and pressure
    ...
    double H2 = cosmo->H0_Mpc* sqrt(OmL + OmM + OmRbar + Omaxionbar_z); // H(z_next)
    double dE, dE2 = (H2-H)/H2/(zi*zstep_log);
...
int findmass_1nu(
    ...
    double* R_solution, 
    double* Mnu_solution){
    //this function finds Mnu(t) given the R(t) input. We use the BKT 
    //approximation for neutrino clustering.
    //mass_0 and temp_0 are the mass and temperature (at z=0) for the particle
    //(neutrino or extra), we specify it to distinguish which case we do.

    double precision_findmass = fmin(fmax(precision_clustering,0.5),2.0); 
    //precision parameter to regulate speed vs. precision.

    long nzpoints=400*precision_findmass;
    //how many points we integrate over in z (or t)
    double zstep;
    //= (z-zi)/(nzpoints-1.); //linear binning in z, since we're interested 
    //in early times
    ...
    double OmRbar, OmM, OmGbar, Omnu_masslessbar, Omnu1, Omnu2, Omnu3; 
    //average for all of them, nu3=extra.
    double OmL=cosmo->OmegaL;

    double rho_ratio_nu1; //ratio of rho(z)/rho(z=0)
    double rho1_0=rho1list_EoS[0] + (cosmo->Omeganu1 == 0) * 1;
    //rho at z=0. units are K^4. add 1 if not present to avoid NaNs
    double rho1_z;

    double rho_ratio_nu2; //ratio of rho(z)/rho(z=0)
    double rho2_0=rho2list_EoS[0] + (cosmo->Omeganu2 == 0) * 1;
    //rho at z=0. units are K^4
    double rho2_z;

    double rho_ratio_nu3; //ratio of rho(z)/rho(z=0)
    double rho3_0=rho3list_EoS[0] + (cosmo->Omega_extra == 0) * 1;
    //rho at z=0. units are K^4
    double rho3_z;


    //all temperatures, mases, and momenta in eV unless otherwise specified 
    //(we'd say: _Mpc):
    ...
    if(debug_mode>0){
        lengthname=sprintf(filename,"tests/Mnu_z-%d.dat",cosmo->file_tag); 
        //we save escape fraction in a file
        fp=fopen(filename,"w");
    }

    //we first find \eta(z), the superconformal time, 
    //\eta(z) = \int dz/(H(z)*(1+z))
    ...
    double factor = 2.0*mass/temp; 
    //x, q, and z-independent factor, 2 because 2 degrees of freedom in f0. 
    //unitless.
    ...
    z_out=zi; 
    //we set it first at z_initial~200 at the begining, for the first test.

    long Nz_solution_max = (
        (log(cosmo->z_collapse)-logz_solution_min)/dlogz_solution
    ); //to make sure we don't continue after collapse.

    for (i=0;(i<Nz_solution) && (i<Nz_solution_max);i++){ 
        //we stop when we cross z_collapse, which is before 
        //zf_code=z_collapse/2. We keep the same array than in collapse code 
        //for convenience.
        Mnu_solution[i] = 0.0;
        z_out = exp(logz_solution_min+dlogz_solution*i); 
        //that's where R_solution is saved, and where we save Mnu_solution.
        eta_out=interpol_cubic(zmin_EoS, dz_EoS, etalist, Nz_EoS, z_out);

        //xmax = (1+z_out)*interpol_cubic(logz_solution_min, dlogz_solution, 
        //R_solution, Nz_solution, log(z_out)); 
        //comoving radius of halo at z_out.
        ...
        for(ix=1;ix<Nx;ix++){//comoving position, we avoid x=0.
            ...
            for(iq=0;iq<Nq;iq++){//comoving momentum
                ...
                factorq = exp(q/temp)/pow(1.+exp(q/temp),2.);  
                //assumming FERMIONS.
                for(imu=0;imu<Nmu;imu++){//cosine between them
                    ...
                    for (iz=0;iz<nzpoints-1;iz++){
                        zstep = (zi-z_out)/(nzpoints-1.); 
                        //we integrate z_inside (z_in) from z_initial (z_i) to
                        //z_outside (z_out).
                        z_in = z_out + zstep * iz;
                        eta_in=interpol_cubic(
                            zmin_EoS, 
                            dz_EoS, 
                            etalist, 
                            Nz_EoS, 
                    ...
                    H = cosmo->H0_Mpc * sqrt(
                        OmL + OmM + OmRbar + Omnu1 + Omnu2 + Omnu3
                    );


                    xc=(eta_out-eta_in)*q/mass;
                    R_halo = interpol_cubic(
                        logz_solution_min, 
                        dlogz_solution, 
                        R_solution, 
                        Nz_solution, 
                    ...
                    Mhalo_smooth = (
                        cosmo->H0_Mpc*cosmo->H0_Mpc*pow(R_halo,3.)*OmM/2.
                    );
                    deltaM = cosmo->Mhalo_Mpc - Mhalo_smooth;
                    //Mhalo-Mhalo_smooth. In Mpc.
                    ...
                    //printf("z_in=%.1le, deltaM=%.1le, xc=%.1le, x_halo=%.1le,
                ...
                Mnu_solution[i] += (
                    (2*PI*x*x*dx) 
                    * (dmu) 
                    * (dq_log * q * q * q /(2.*PI*PI) ) 
                    * f1[ix][iq][imu]
                ); //\int d^3x d^3q = dx x^2 (2PI) dmu dp p^2/(2PI^2).
                }//mu-loop
            }//q-loop
        }//x-loop

        Mnu_solution[i]*=mass*eVtoMpc/pow(hbareVMpc,3.); 
        //to get Mnu in Mpc we need hbar^3 (that is why there is a 1/(2pi)^3 in p 
        //integral). There are 3 q in eV to convert to Mpc-1 (since x is in Mpc),
        ...
        Mnusmooth = (
            ...
            * cosmo->H0_Mpc
            * pow(R_solution[i],3.)
            * (cosmo->Omeganu1+cosmo->Omeganu2+cosmo->Omega_extra)/2.
            * pow(1+z_out,3.)
        );
        ...
        if(debug_mode>0){
            printf(
                "z=%.1le, Mnu/Mhalo=%1le \n",
                z_out,
                Mnu_solution[i]/cosmo->Mhalo_Mpc
            );
            fprintf(
                fp, 
                "%le  %le  %le \n",
                z_out, 
                Mnu_solution[i]/cosmo->Mhalo_Mpc, 
                (Mnu_solution[i]+Mnusmooth)/cosmo->Mhalo_Mpc
            );
        }
    }
