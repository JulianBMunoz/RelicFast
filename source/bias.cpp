...
//which z_collapse are we in:
	int number_collapse=find_value(cosmo->N_zcoll,cosmo->z_collapse_array,cosmo->z_collapse);
	int j_collapse = cosmo->j_collapse_array[number_collapse];
	if (debug_mode>0) printf("(in bias) number_collapse=%d, j_collapse=%d \n",number_collapse,j_collapse);
...
//there isn't a lot to win from looping over z inside here, as opposed to outside, since you still have to read Boltzmann output every time.
	...
	////////////////////////////////////////
		...
		double *transfer_array_gamma_z_coll, *transfer_array_nu_massless_z_coll, *transfer_array_nu1_z_coll; //for photons, massless neutrinos, and massive neutrino1
		...
		double *transfer_array_nu2_z_coll, *transfer_array_extra_z_coll; //for mnu2 and extra species
		...
		transfer_check=gettransfer_matter(cosmo, filename, k_transfer_array, transfer_array_z_coll);
		...
		transfer_check=gettransfer_nu_massless(cosmo, filename, k_transfer_array, transfer_array_nu_massless_z_coll);
		do_check(transfer_check);
		//and for massive neutrino 1
		if(cosmo->mnu1>0){
			transfer_check=gettransfer_nu1(cosmo, filename, k_transfer_array, transfer_array_nu1_z_coll);
			do_check(transfer_check);
		}
		//and for massive neutrino 2
		if(cosmo->mnu2>0){
			transfer_check=gettransfer_nu2(cosmo, filename, k_transfer_array, transfer_array_nu2_z_coll);
			do_check(transfer_check);
		}
		//and for extra species
		if(cosmo->Omega_extra > 0){
			transfer_check=gettransfer_extra(cosmo, filename, k_transfer_array, transfer_array_extra_z_coll);
			do_check(transfer_check);
		}


	//we also save the transfer function from CLASS/CAMB at z_collapse in the output folder.
		if(boltzmann_tag == 0){//CLASS
			lengthname=sprintf(filename,"cp Boltzmann_%d/transfer_files_%d/z%d_tk.dat output/result-%d/z%.2ftk.dat",
									boltzmann_tag, cosmo->file_tag, j_collapse+1, cosmo->file_tag, cosmo->z_collapse); //zlist reversed, starts at 1
		}
		else { //CAMB
			lengthname=sprintf(filename,"cp Boltzmann_%d/transfer_files_%d/_transfer_out_z%.3f output/result-%d/z%.2fTF_CAMB.dat",
													boltzmann_tag, cosmo->file_tag, cosmo->z_collapse, cosmo->file_tag, cosmo->z_collapse); //We reuse the same filename variable name.
		}
		system(filename);
	...
	//b_E = Phm/Pmm.
		...
		const int N_species = 1 + cosmo->counter_massive_nus + (cosmo->Omega_extra>0);//how many species (cdm+b) + others
		int species_counter=0;


		const double Omegatot = cosmo->OmegaM + cosmo->Omeganu1 + cosmo->Omeganu2 + cosmo->Omega_extra; //remember we call OmegaM=Omegab+Omegac
		...
		Power_spectra=allocate_2D_array(N_species,N_species); // cdm, nu1, nu2, extra.
  ...
  int iM;
	...
	///////////////////////////////////////////
		...
		if(print_headers!=0){
			fgets(buffer, 100, fp); //reads 100 characters in first line. To get rid of headers.
		}
		for(i_delta_long=0;i_delta_long<N_delta_long;i_delta_long++){
			for(i_klong=0;i_klong<cosmo->N_klong;i_klong++){
				fscanf(fp,"%le %le %le", &delta_long_collapse[i_delta_long][i_klong], &klong_list[i_klong], &delta_short_crit[i_delta_long][i_klong]);
			}
		}
		fclose(fp);
...
//we find the "average" value for delta_crit_sim for our collapse. Just to get a fair estimate of the amplitude of b_1^L
	...
	//	printf("HMF = %le \n",HMF);
		...
		for(i_klong=0;i_klong<cosmo->N_klong;i_klong++){
			derivative=(delta_short_crit[N_delta_long-1][i_klong]-delta_short_crit[0][i_klong])/
					(delta_long_collapse[N_delta_long-1][i_klong]-delta_long_collapse[0][i_klong]); // ddelta_crit/ddelta_long
		...
		for(i_klong=0;i_klong<cosmo->N_klong;i_klong++){
			...
			factor=(2.*PI*PI)*cosmo->As*pow(k_long/kpivot,cosmo->ns-1.)/k_long/k_long/k_long;
			Pmm[i_klong] = Pmh[i_klong] = Phh[i_klong] = 0.;

			tf_species[0] = interpol(transfer_array_z_coll,k_transfer_array,length_transfer,k_long);

			if(cosmo->mnu1>0){
			species_counter++;
			tf_species[species_counter] = interpol(transfer_array_nu1_z_coll,k_transfer_array,length_transfer,k_long);
			}
			if(cosmo->mnu2>0){
				species_counter++;
				tf_species[species_counter] = interpol(transfer_array_nu2_z_coll,k_transfer_array,length_transfer,k_long);
			}
			if(cosmo->Omega_extra>0){
				species_counter++;
				tf_species[species_counter] = interpol(transfer_array_extra_z_coll,k_transfer_array,length_transfer,k_long);
			}
			species_counter=0; //we reset the counter.
			...
			for(i=0;i<N_species;i++){
				for(j=0;j<N_species;j++){
					...
					Pmm[i_klong]+= Power_spectra[i][j] * fraction_species[i] * fraction_species[j]; //P_mm
	//				printf("k/h=%.3le, i=%d, j=%d, P=%.1le \n",k_long/h,i,j,Power_spectra[i][j]);
				}
				Pmh[i_klong]+= (1.0+b_L[i_klong]) * Power_spectra[i][0] * fraction_species[i]; //P_mhalo
			}
			Phh[i_klong]+= (1.0+b_L[i_klong]) * (1.0+b_L[i_klong]) * Power_spectra[0][0]; //P_hh = (1+bL)^2Pcc


			b_E[i_klong]=Pmh[i_klong]/Pmm[i_klong];

			if(N_species>1 && debug_mode>0){
				printf("k/h=%.1le,  Tx/Tc=%.3le, b_E=%.3le, b_L=%.3le \n",k_long/cosmo->h, Power_spectra[1][0]/Power_spectra[0][0], Pmh[i_klong]/Pmm[i_klong], b_L[i_klong]);
			}
	...
	////////////////////////////////////
		...
		lengthname=sprintf(filename,"output/result-%d/bias_Lagrangian_z%.2f_M%.2f_Nk%d.dat",cosmo->file_tag,cosmo->z_collapse, log10(cosmo->Mhalo),cosmo->N_klong); //we save the delta_crit extrapolated to z=0.5
		fp=fopen(filename,"w");
		...
		lengthname=sprintf(filename,"output/result-%d/bias_Euler_z%.2f_M%.2f_Nk%d.dat", cosmo->file_tag, cosmo->z_collapse, log10(cosmo->Mhalo),cosmo->N_klong); //we save the delta_crit extrapolated to z=0.5
		fp=fopen(filename,"w");
		...
		lengthname=sprintf(filename,"output/result-%d/power_spectra_z%.2f_M%.2f_Nk%d.dat", cosmo->file_tag, cosmo->z_collapse, log10(cosmo->Mhalo),cosmo->N_klong); //we save the delta_crit extrapolated to z=0.5
		fp=fopen(filename,"w");
		...
		for(i_klong=0;i_klong<cosmo->N_klong;i_klong++){
			k_long=klong_list[i_klong];
			fprintf(fp,"%.4le\t %.4le\t %.4le\t %.4le \n", k_long, Pmm[i_klong], Pmh[i_klong], Phh[i_klong]);
		}
		fclose(fp);
