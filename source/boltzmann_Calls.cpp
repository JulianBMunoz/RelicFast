...
int tests_pre_run(Cosmology *cosmo){
	if(boltzmann_tag == _CAMB_){//if camb:
		...
		else if(cosmo->counter_massive_nus == 1 && cosmo->tag_sterile_nu + cosmo->tag_thermal_relic != 0){
...
//some coding tests:
	double mnu_low_limit=0.009;//eV, how light can nus be before they are (somewhat) relativistic at z=0 (T0~0.1meV, so for 0.1 eV we are safe).
	if(cosmo->counter_massive_nus>0){
		...
		if(cosmo->Neff + cosmo->counter_massive_nus > 4){
			printf("Are you sure you want Neff~3 AND massive neutrinos? If so change YHe=0.24 in CLASS. \n");
		}
	...
	if(cosmo->tag_sterile_nu != 0){
		if(cosmo->m_SN<mnu_low_limit){
			printf("Error in sterile neutrino specifications, particle has to be non-relativistic at z=0. \n");
			return -1;
		}
	}
	...
	if(cosmo->tag_thermal_relic != 0){
		if(cosmo->m_TR<cosmo->T0_TR*KtoeV){
			printf("Error in thermal relic specifications, particle has to be non-relativistic at z=0. \n");
			return -1;
		}
		z_rel = cosmo->m_TR/(cosmo->T0_TR*KtoeV);

		if(pow(cosmo->T0_TR/cosmo->T0_nu,3.)*cosmo->m_TR/(cosmo->T0_nu*KtoeV)/z_rel > 1.){
	...
	if(cosmo->z_collapse_top > 1. && cosmo->N_zcoll>1 && HMF_option!=_ST_){
		printf("For z>1 HMFs from simulations might not properly calibrated, change to Sheth-Tormen HMF. \n");
	}
...
int gettransfer_matter(Cosmology *cosmo, char *filename,  double *kgrid, double *TF){
	...
	//	We need to give the right values to TF, evaluated over k and not k/h, we define kgrid for that //
		for(j=0;j<length_transfer;++j){
			...
			TF[j] = cosmo->omegab/(cosmo->omegab+cosmo->omegac) * TF_grid_b[j] + cosmo->omegac/(cosmo->omegab+cosmo->omegac) * TF_grid_c[j];//in class it's not over k^2
		}
...
//this function extracts the transfer function of photons, make sure that the column is right if you use a different CLASS/CAMB version.
...
//this function extracts the transfer function of massless neutrinos, make sure that the column is right if you use a different CLASS/CAMB version.
...
//this function extracts the transfer function of neutrino 1, make sure that the column is right if you use a different CLASS/CAMB version.
...
//this function extracts the transfer function of extra species, make sure that the column is right if you use a different CLASS/CAMB version.
...
int save_transfers_camb(Cosmology *cosmo, double zlist_transfer[], double *kgrid,
					double **TFm, double **TFgamma, double **TFnu, double **TFnu_mass, double **TFnu2, double **TFextra){
...
//nu2 and extra are set to be identical to nu1 if all are active.
	...
	int checkm, checknu, checknu_mass, checkgamma;
 	...
 	for(jz=0;jz<Nz_transfer;jz++){
 		lengthname=sprintf(filename,"Boltzmann_%d/transfer_files_%d/_transfer_out_z%.3f", boltzmann_tag, cosmo->file_tag, zlist_transfer[jz]);
		fp=fopen(filename, "r");
...
int save_transfers_class(Cosmology *cosmo, double zlist_transfer[], double *kgrid,
...
//we will save the TF from CLASS
	...
	int checkm, checknu, checknu_mass, checkgamma;
 	...
 	for(jz=0;jz<Nz_transfer;jz++){
		lengthname=sprintf(filename,"Boltzmann_%d/transfer_files_%d/z%d_tk.dat", boltzmann_tag, cosmo->file_tag, Nz_transfer-jz);//names start at z1=z_i, up to Nz_transfer-1 = z0. zlist transfer has been reversed.
		fp=fopen(filename, "r");
		checkm=gettransfer_matter(cosmo, filename, kgrid, TFm[jz]);
		do_check(checkm);
		checkgamma=gettransfer_gamma(cosmo, filename, kgrid, TFgamma[jz]);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
//// These functions run Boltzmann over the redshift list zlist_transfer and saves transfer functions.  ////
///////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//create appropriate folder for CAMB output
	int lengthfilename=200;
 	char *folder_name;
 	folder_name=(char *)malloc((lengthfilename+1)*sizeof(char));	//	lengthfilename=sprintf(folder_name,"transfer_files_%d/test", file_tag); // 	fp=fopen(folder_name, "w");// 	fclose(fp);
	lengthfilename=sprintf(folder_name,"mkdir Boltzmann_%d", boltzmann_tag);
	system(folder_name);
	lengthfilename=sprintf(folder_name,"mkdir Boltzmann_%d/transfer_files_%d", boltzmann_tag, cosmo->file_tag);
	system(folder_name);
...
//	system("make");
	...
	system("CAMB_Current/./camb CAMB_Current/params_collapse.ini > output_cmd.txt");
...
//create appropriate folder for CAMB output
	...
	fprintf(fp, "%.3f \n", zlist_transfer[Nz_transfer-1]); //last one apart, to not have a comma

	fprintf(fp, "root = Boltzmann_%d/transfer_files_%d/ \n", boltzmann_tag, cosmo->file_tag);
...
//	system("make");
	...
	system("mv explanatory_collapse.ini CLASS_Current/explanatory_collapse.ini");
	lengthfilename=sprintf(folder_name,"CLASS_Current/./class CLASS_Current/explanatory_collapse.ini >	Boltzmann_%d/transfer_files_%d/output_cmd.txt", boltzmann_tag, cosmo->file_tag);
	system(folder_name);
...
/////////////////////////////////////////////////////////////////////////////////////////
////    we RUN Boltzmann code to get the transfer functions between z=zi and z=0.	////
////////////////////////////////////////////////////////////////////////////////////////

int boltzmann(Cosmology *cosmo, double *zlist_transfer){
//this function runs Boltzmann code (CLASS or CAMB), given z_collapse_array as input,
//and stores in j_collapse_array the numbers of those z_collapses (since CLASS does not save z in output)
//and saves the list of zs in zlist_transfer.

		double *zlist_transfer_code;//same a zlist_transfer but reversed so it goes upwards. Because of CAMB...
		...
		const double zstep_sqrt_transfer=(sqrt(zf)-sqrt(zi))/(Nz_transfer-1); //for reading the transfer functions
		//we use sqrt(z) spacing, since it samples better low z, and we cannot do log-spacing since we want to get to z=0.

		for(jz=0;jz<Nz_transfer;jz++){
			zlist_transfer_code[jz] = pow(sqrt(zi)+zstep_sqrt_transfer*jz,2.);	//	printf("%le \n",zlist_transfer_code[jz]);
		}



	//we tweak the zlist a bit
	//first we make sure the first z after zi is close to have accurate derivative
		double epsilon_z=0.005; // dz/z at z_i to compute derivative, 0.005 is good, not terribly important, since it's a small correction.
		zlist_transfer_code[0]=zi*(1.+epsilon_z); //we overwrite the first non-zi z, for derivative at zi to be accurate.
		zlist_transfer_code[1]=zi;
		zlist_transfer_code[2]=zi*(1.-epsilon_z); //we also overwrite the third non-zi z, for derivative at zi to be more accurate.
  ...
  //and now we check that the z are not "colliding", and have different values.
		...
		for(jz=0;jz<Nz_transfer;jz++){
			zlist_transfer_code[jz] = (int)(zlist_transfer_code[jz] * 1000) / 1000.0f; //we do this so the calculated values are exactly the ones with save, with the .3f
		}
		...
		if(boltzmann_tag == 0){//CLASS
			...
			printf("Choose either CLASS or CAMB \n");
			return -1;
		}
		...
		//we reverse zlist_transfer now so it's ordered up (CAMB requires it to be ordered down...)
		for(jz=0;jz<Nz_transfer;jz++){
			zlist_transfer[jz]=zlist_transfer_code[Nz_transfer-1-jz];	//		printf("%le \n",zlist_transfer[jz]);
		}


	//we save the zlist to a file if we use CLASS.
		if(boltzmann_tag == 0){//CLASS
			lengthname=sprintf(filename,"Boltzmann_%d/transfer_files_%d/zlist.dat",boltzmann_tag,cosmo->file_tag); //zlist for every tk
			fp=fopen(filename,"w");
			if(print_headers!=0){
...
//	cout << buffer << "\n";
	...
	{"N_Mhalo", "N_zcoll", "N_klong",
		"tag_thermal_relic", "tag_sterile_nu",
		"Mhalo_min", "Mhalo_max",
		"z_collapse_bot", " z_collapse_top",
		"kbot", "ktop",
		"m_TR", "omega_TR", "m_SN",
		"omegab", "omegac", "hubble", "A_s", "n_s",
		"mnu1", "mnu2", "N_eff_input"};
...
//reads parameter value from buffer, after finding parameter_name
	...
	int length_line = 50;
	char line[length_line]; //temporary variable, to store the line where the variable name appears

	std::size_t position = buffer.find(parameter_name); //we find parameter_name in buffer


	for(i=0;i<length_line;i++){
		line[i]	= buffer.at(position+i); //we save the parameter name and its value in temporary line
	}
	...
	sscanf(line, "%*[^=]= %le ", &parameter_value); //save the number after the equal sign
...
//we assign the read parameters.
	...
	cosmo->counter_massive_nus = (cosmo->mnu1>0) + (cosmo->mnu2>0); //we subtract one from Neff per neutrino, and we use it for counting columns in CLASS

	cosmo->Neff = cosmo->Neff_input - constant_change_Neff * (cosmo->counter_massive_nus + cosmo->tag_sterile_nu*(cosmo->m_SN<m_SN_thresh_Neff) ); //we also subtract 1 if SN is below thresh mass, as we assume it's a 3rd regular neutrino->


	cosmo->file_tag = (cosmo->tag_thermal_relic>0) * (int)(1000.*cosmo->omega_TR + 1000*cosmo->m_TR + 0.5f)
								+(cosmo->tag_sterile_nu>0) * (int)(100*cosmo->m_SN + 0.5f); //some placeholder for what to use as filetag. + 0.5f so it rounds



	cosmo->omega_SN = cosmo->m_SN/mass_constant_nu; //Omega h^2, since Temperature is the same than regular nus.


	cosmo->omega_extra = (cosmo->tag_thermal_relic>0)*cosmo->omega_TR + (cosmo->tag_sterile_nu>0)*cosmo->omega_SN;
	cosmo->Omega_extra = cosmo->omega_extra/cosmo->h/cosmo->h;
	cosmo->m_extra = (cosmo->tag_thermal_relic>0)*cosmo->m_TR + (cosmo->tag_sterile_nu>0)*cosmo->m_SN;
	...
	cosmo->OmegaM=(cosmo->omegab+cosmo->omegac)/cosmo->h/cosmo->h; //b+c energy density, this is what haloes are made of (and what enters R_M)

	cosmo->T0_photons = 2.7255; //temperature of photons today, in K, from Planck2015 (I assume this is not going to be changed often)
	cosmo->T0_nu = cosmo->T0_photons * ratio_T_nu_T_gamma;//temperature of neutrinos today, in K.


	cosmo->T0_TR = (cosmo->tag_thermal_relic>0) ? cosmo->T0_nu * pow(cosmo->omega_TR * mass_constant_nu/cosmo->m_TR, 1./3) : 0.0; //temperature of TR today in K. //Fixed through cosmic abundance, assuming relativistic at decoupling and non-rel today
	cosmo->T0_extra = (cosmo->tag_sterile_nu>0)*cosmo->T0_nu + (cosmo->tag_thermal_relic>0)*cosmo->T0_TR; //SN have nu temperature.


	cosmo->OmegaG = omega_constant/cosmo->h/cosmo->h * pow(cosmo->T0_photons,4.0);//Omega_photons at z=0
	cosmo->Omeganu_massless = cosmo->OmegaG * 7./8 * cosmo->Neff * pow(cosmo->T0_nu/cosmo->T0_photons,4.0); //Omega_masslessneutrinos
	cosmo->OmegaR = cosmo->OmegaG + cosmo->Omeganu_massless; //radiation (at all z)


	cosmo->OmegaL = 1.0 - cosmo->OmegaM - cosmo->Omeganu1 - cosmo->Omeganu2 - cosmo->OmegaR - cosmo->Omega_extra; //close the Friedmann eq., no curvature.

	cosmo->H0_Mpc=100.*cosmo->h/(c_light); //H0 in Mpc-1
...
//redshifts:
	cosmo->j_collapse_array = allocate_1D_array_int(cosmo->N_zcoll); //these are the indices, we find them with boltzmann()
	cosmo->z_collapse_array = allocate_1D_array(cosmo->N_zcoll);
	int iz;
	double z_collapse_step=0.;
	if(cosmo->N_zcoll>1){
		z_collapse_step=(cosmo->z_collapse_top-cosmo->z_collapse_bot)/(cosmo->N_zcoll-1.);
	}
