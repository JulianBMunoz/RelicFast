...
for mnu_idx, mnu_val in enumerate(Mnu): 
    ...
    plt.plot(kplot, eulbiasplot/eulbiasplot[0], label=r'$m_{\nu, i}=$'+f'{mnu_val/3.:.3e}'+r' eV', color=tuple(rgb))

plt.plot([0.7*0.015, 0.7*0.015], [0.999, 1.008], color='red', label=r'$k_{eq}$')
plt.plot([0.024, 0.024], [0.999, 1.008], color='blue', label=r'$k_{*}$')
plt.xlabel(r'$k ~[{\rm Mpc}^{-1}]$', fontsize=15)
plt.ylabel(r'$b_1(k)/b_1(k_{\rm ref})$', fontsize=15)
plt.title(r'$\omega_\chi = \omega_{\nu},  ~z = 0.65, ~m_\chi = 10^{-30}$ eV', fontsize=15)
...
kvals_chi = data_pm[0][:, 0]
pmvals_chi = data_pm[0][:, 1]

kvals_nu = data_pm[1][:, 0]
pmvals_nu = data_pm[1][:, 1]

pminterp_chi = scipy.interpolate.interp1d(kvals_chi, pmvals_chi)
pmplot_chi = pminterp_chi(kplot)

pminterp_nu = scipy.interpolate.interp1d(kvals_nu, pmvals_nu)
pmplot_nu = pminterp_nu(kplot)

yplot = pmplot_chi/pmplot_nu

plt.plot(kplot, yplot, label=r'$m_{\nu, i}=$'+f'{mnu_val/3.:.3e}'+r' eV', color=tuple(rgb))

plt.plot([0.7*0.015, 0.7*0.015], [min(yplot), max(yplot)], color='red', label=r'$k_{eq}$')
plt.plot([0.024, 0.024], [min(yplot), max(yplot)], color='blue', label=r'$k_{*}$')
plt.xlabel(r'$k ~[{\rm Mpc}^{-1}]$', fontsize=15)
plt.ylabel(r'$P_{m, \chi\Lambda CDM}(k)/P_{m, \nu\Lambda CDM}(k_{\rm ref})$', fontsize=15)
plt.title(r'$\omega_\chi = \omega_{\nu},  ~z = 0.65, ~m_\chi = 10^{-30}$ eV', fontsize=15)
#plt.legend(lines, labels, fontsize=15)
plt.legend(fontsize=15)
#plt.yscale('log')
